...
namespace Dddesignkit.Tests.Http
{
    public class ApiInfoParserTests
    {
        public class TheParseApiHttpHeadersMethod
        {
            ...
            public void BadHeadersAreIgnored()
            {
                var headers = new Dictionary<string, string>
                {
                    {
                        "Link",
                        "<https://api.dribbble.com/v1/shots?page=4&per_page=5>; , " +
                        "<https://api.dribbble.com/v1/shots?page=131&per_page=5; rel=\"last\""
                    }
                };
            ...
            public void ParsesLinkHeader()
            {
                var headers = new Dictionary<string, string>
                {
                    {
                        "Link",
                        "<https://api.dribbble.com/v1/shots?page=4&per_page=5>; rel=\"next\", " +
                        "<https://api.dribbble.com/v1/shots?page=131&per_page=5>; rel=\"last\", " +
                        "<https://api.dribbble.com/v1/shots?page=1&per_page=5>; rel=\"first\", " +
                        "<https://api.dribbble.com/v1/shots?page=2&per_page=5>; rel=\"prev\""
                    }
                };
                ...
                Assert.Equal(new Uri("https://api.dribbble.com/v1/shots?page=4&per_page=5"),
                    apiInfo.Links["next"]);
                Assert.Contains("prev", apiInfo.Links.Keys);
                Assert.Equal(new Uri("https://api.dribbble.com/v1/shots?page=2&per_page=5"),
                    apiInfo.Links["prev"]);
                Assert.Contains("first", apiInfo.Links.Keys);
                Assert.Equal(new Uri("https://api.dribbble.com/v1/shots?page=1&per_page=5"),
                    apiInfo.Links["first"]);
                Assert.Contains("last", apiInfo.Links.Keys);
                Assert.Equal(new Uri("https://api.dribbble.com/v1/shots?page=131&per_page=5"),
                    apiInfo.Links["last"]);
            }
        }
        ...
        public class ThePageUrlMethods
        {
            ...
            {
                var pageUri = new Uri("https://api.dribbble.com/v1/shots?page=2");
                var links = new Dictionary<string, Uri>
                {
                    { "garbage", new Uri("https://api.dribbble.com/v1/shots?page=1") },
                    { linkName, pageUri }
                };
